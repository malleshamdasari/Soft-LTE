...
void nas::open_ul_sock()
{
  ul_sock_fd = socket(AF_INET, SOCK_DGRAM, 0);
  if (ul_sock_fd < 0) 
    cout<<"ERROR opening socket";
}

void nas::close_ul_sock()
{
  if(close(ul_sock_fd) == -1) 
...
void nas::write_pdu_sock(uint8_t *msg, int N_bytes, uint8_t msg_type)
{
  socklen_t             serverlen;
  ...
  *temp = msg_type;
  ...
  { 
    ...
    printf("UE sent message: %d\n",bytes_sent);
}

void nas::handle_dlnastransport(char *msg, int len)
{ 
...
void nas::write_pdu(uint32_t lcid, byte_buffer_t *pdu) {
  ...
  printf("NAS message with MSG_TYPE=%02X\n", msg_type);
...
void nas::parse_attach_accept(uint32_t lcid, byte_buffer_t *pdu) {
  ...
  if (attach_accept.eps_attach_result == LIBLTE_MME_EPS_ATTACH_RESULT_EPS_ONLY) {
    ...
    printf("Sending Attach Complete: %d \n", pdu->N_bytes);
...
void nas::parse_security_mode_command(uint32_t lcid, byte_buffer_t *pdu)
{
  ...
  //if (integrity_check(pdu) != true) { Mallesh, for now.
  ...
  cout<<"bytes before encrypt: "<<pdu->N_bytes<<"\n";
  cipher_encrypt(pdu);
  cout<<"bytes after encrypt: "<<pdu->N_bytes<<"\n";
  integrity_generate(&k_nas_int[16],
                     ctxt.tx_count,
                     SECURITY_DIRECTION_UPLINK,
                     &pdu->msg[5],
...
void nas::send_attach_request() {
  ...
  write_pdu_sock(msg->msg, msg->N_bytes, 2);
...
void nas::send_security_mode_reject(uint8_t cause) {
  ...
  //rrc->write_sdu(cfg.lcid, msg);
  write_pdu_sock(msg->msg, msg->N_bytes, 1);
}
