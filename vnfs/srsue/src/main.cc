...
void *sock_rx_dl(void *m)
{
  ...
  while(1)
  {
    ...
    {
        cout<<"DL message received from eNodeB\n";
        ue->sock_rx_dl_handler(buf, n); 
    }
  }
...
int main(int argc, char *argv[])
{
  ...
  pthread_t dlsock_thread;
  pthread_create(&dlsock_thread, NULL, &sock_rx_dl, &args);
