...
#include "srsenb/hdr/controller_if.h"
...
void parse_args(all_args_t *args, int argc, char* argv[]) {
  ...
  common.add_options()
    ...
    ("enb.mme_addr",      bpo::value<string>(&args->enb.s1ap.mme_addr)->default_value("204.178.7.45"),"IP address of MME for S1 connnection")
...
#define MAX_MMES 10 /* Max supported MMEs from eNodeB side. 
...
void *setup_mme(void *buf)
{
  ...
  /* Extract the configuration parameters from controller interface i.e ONAP */
  n = msg[0];
  msg += sizeof(char);
  mme_cntxt = (mme_cntxt_t *)msg;
  msg = msg + sizeof(mme_cntxt_t);
  unsigned char *t = (unsigned char *)malloc(n-sizeof(mme_cntxt_t));
  memcpy(t, msg, n-sizeof(mme_cntxt_t));

  /* Convert the IP from char * to string */
  for(int i = 0; i < (n-sizeof(mme_cntxt_t)); i++)
    buffer << t[i];
  mme_ip = buffer.str();
  mme_port = mme_cntxt->mme_port;

  cout<<"Setting up MME with IP: "<<mme_ip<<" and Port: "<<mme_port<<"\n";

  /* Setup MME with the given configuration parameters */
  enb->setup_mme( mme_port, mme_ip );

  return 0;
}

void *instantiate_mme( void *m )
{
  int8_t  		i, n; 
  int			size;
  int 			sockfd; 
  ...
  while(1)
  {
    cout<<"Waiting for configuration of new MME from controller interface\n";
    ...
    else
    {
      msg = (char *)malloc(n+1);
      msg[0] = n;
      memcpy(msg+sizeof(char), buf, n);
      pthread_create(&mme_thread[thread_id], NULL, &setup_mme, (void *)msg);
      thread_id++;
    }
  }
...
void *sock_rx_ul(void *m)
{
  ...
  while(1)
  {
    n = recvfrom(sockfd, buf, 1600, 
                MSG_WAITALL, ( struct sockaddr *) &clientaddr, &clientlen);
    if (n < 0)
      printf("ERROR in recvfrom\n");
    else
    {
        cout<<"Message received from UE\n";
        enb->sock_rx_ul_handler(buf, n);
    }
  }
...
int main(int argc, char *argv[])
{
  ...
  pthread_t vnfthread;
  pthread_create(&vnfthread, NULL, &instantiate_mme, &args);

  pthread_t ulsock_thread;
  pthread_create(&ulsock_thread, NULL, &sock_rx_ul, &args);
  ...
  pthread_cancel(vnfthread);
  pthread_cancel(ulsock_thread);
  metrics.stop();
  enb->stop();
  enb->cleanup();
  cout << "---  exiting  ---" << endl;
