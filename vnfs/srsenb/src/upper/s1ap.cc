...
void s1ap::open_dl_sock()
{
  dl_sock_fd = socket(AF_INET, SOCK_DGRAM, 0);
  if (dl_sock_fd < 0)
    cout<<"ERROR opening socket";
}

void s1ap::close_dl_sock()
{
  if(close(dl_sock_fd) == -1)
    cout<<"Failed to close SCTP socket\n";
}
...
void s1ap::spin_up_mme(vmme_cntxt_t *vmme_cntxt)
{
  ...
  // Connect to MME
  while(running && !connect_vmme(vmme_cntxt)) {
  ...
  while(running) {
    ...
    cout<<"eNodeB Received S1AP PDU from MME \n";
...
bool s1ap::connect_vmme(vmme_cntxt_t *vmme_cntxt)
{ 

  cout<<"Previous socket fd: "<<socket_fd<<"\n";

  socket_fd = 0;

  cout<<"Connecting to MME on IP: "<<vmme_cntxt->mme_addr.c_str()<<" and Port: "<<vmme_cntxt->mme_port<<"\n";
...
bool s1ap::connect_mme()
{
  ...
  cout<<"Connecting to MME "<<args.mme_addr.c_str()<<MME_PORT<<"\n";
...
void s1ap::write_pdu_sock(uint8_t *msg, int N_bytes, uint8_t msg_type)
{
  ...
  *temp = msg_type;
  ...
  if (bytes_sent < 0)
  {
    ...
    printf("eNodeB sent DL message to UE: %d\n",bytes_sent);
}

bool s1ap::handle_initiatingmessage(LIBLTE_S1AP_INITIATINGMESSAGE_STRUCT *msg)
{
  cout<<"Intiating message: "<<liblte_s1ap_initiatingmessage_choice_text[msg->choice_type]<<"\n";
...
bool s1ap::handle_initialctxtsetuprequest(LIBLTE_S1AP_MESSAGE_INITIALCONTEXTSETUPREQUEST_STRUCT *msg)
{
 ...
 // res.E_RABSetupListCtxtSURes.len = 1;
 // res.E_RABFailedToSetupListCtxtSURes.len = 0;

 // res.E_RABSetupListCtxtSURes.buffer[0].ext = false;
 // res.E_RABSetupListCtxtSURes.buffer[0].iE_Extensions_present = false;
 // res.E_RABSetupListCtxtSURes.buffer[0].e_RAB_ID.ext = false;
 // res.E_RABSetupListCtxtSURes.buffer[0].e_RAB_ID.E_RAB_ID = 5;
 // uint32_to_uint8(1, res.E_RABSetupListCtxtSURes.buffer[0].gTP_TEID.buffer);
 //
 // ue_ctxt_setup_complete(1, &res);
 // printf("Sending Attach complete to UE: %d\n", sizeof(LIBLTE_S1AP_MESSAGE_INITIALCONTEXTSETUPREQUEST_STRUCT));
 // write_pdu_sock((uint8_t *)msg, sizeof(LIBLTE_S1AP_MESSAGE_INITIALCONTEXTSETUPREQUEST_STRUCT));

  //Mallesh for now. // Setup UE ctxt in RRC
...
bool s1ap::send_initialuemessage(uint16_t rnti, srslte::byte_buffer_t *pdu, bool has_tmsi, uint32_t m_tmsi, uint8_t mmec)
{
  ...
  cout<<"pdu n bytes: "<<pdu->N_bytes<<"\n";

  // NAS_PDU
  memcpy(initue->NAS_PDU.buffer, pdu->msg, pdu->N_bytes);
  //memcpy(initue->NAS_PDU.buffer, pdu, pdu->N_bytes);
  ...
  cout<<"after pack in send_initialuemessage\n";
...
bool s1ap::send_ulnastransport(uint16_t rnti, srslte::byte_buffer_t *pdu)
{
  ...
  cout<<"Before sent message to MME\n";
  ssize_t n_sent = sctp_sendmsg(socket_fd, msg.msg, msg.N_bytes,
                                (struct sockaddr*)&mme_addr, sizeof(struct sockaddr_in),
                                htonl(PPID), 0, ue_ctxt_map[rnti].stream_id, 0, 0);
  if(n_sent == -1) {
    cout<<"Failed to send message to MME\n";
    s1ap_log->error("Failed to send UplinkNASTransport for RNTI:0x%x\n", rnti);
    return false;
  }
  cout<<"Successfully sent message to MME\n";
