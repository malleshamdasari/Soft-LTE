...
void enb::setup_mme(int mme_port, std::string mme_ip)
{
  s1ap.vmme_cntxt.mme_addr = mme_ip;
  s1ap.vmme_cntxt.mme_port = mme_port;
  s1ap.spin_up_mme(&s1ap.vmme_cntxt);
}

void enb::sock_rx_ul_handler(char *msg, int len)
{
  s1ap.handle_ulnastransport(msg, len);
}
